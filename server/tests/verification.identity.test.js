...
describe('Identity verification', () => {
  test('verify-ownership returns isOwner true when matches', async () => {
    const res = await request(app).post('/api/verification/verify-ownership').send({ tokenId: '0.0.x', serialNumber: '1', accountId: '0.0.7174400' });
  ...
  test('verify-holder-signature returns verified true', async () => {
    const res = await request(app).post('/api/verification/verify-holder-signature').send({ accountId: '0.0.7174400', message: 'nonce-123', signature: 'c2ln' });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
