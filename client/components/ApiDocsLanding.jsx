    ...
    <div className="container-responsive pb-10 pt-24 sm:pt-32">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card p-6">
          ...
          <pre className="text-xs font-mono bg-gray-50 p-3 rounded border overflow-x-auto">{`curl -X POST http://localhost:3001/api/v1/verification/verify-credential \
  -H 'Content-Type: application/json' \
  -d '{"tokenId":"0.0.123456","serialNumber":"1"}'`}</pre>
        </div>
        <div className="card p-6">
          <div className="font-semibold mb-2">Emisión con API Key</div>
          <pre className="text-xs font-mono bg-gray-50 p-3 rounded border overflow-x-auto">{`curl -X POST http://localhost:3001/api/v1/credentials/issue \
