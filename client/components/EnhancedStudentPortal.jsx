...
function EnhancedStudentPortal({ demo = false }) {
  ...
  const renderConnectionStatus = () => {
    const statusConfig = {
      checking: { text: 'Sincronizando...', color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
      connected: { text: 'Red Activa', color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
      demo: { text: 'Modo Simulación', color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/20' },
      error: { text: 'Error de Red', color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/20' }
    };
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#050505]"><div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-emerald-500"></div></div>;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-100 flex flex-col relative overflow-hidden font-sans">
      ...
      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-[#0d0d0d]/40 border-b border-white/5 shadow-sm p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-3">
                <img
                  ...
                  className="h-10 w-10 rounded-full shadow-sm object-contain bg-black border border-white/10"
                />
                <div className="flex flex-col">
                   <h3 className="text-lg font-bold text-white leading-none tracking-tight">{institutionName}</h3>
                   <p className="text-[10px] text-slate-400 font-medium mt-0.5">Impulsado por {institutionName}</p>
                </div>
             </div>
             ...
             <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                 <h1 className="hidden md:block text-lg font-bold text-slate-200">Portal Académico</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="h-10 w-10 rounded-full border border-white/10 p-1">
              <div className="h-full w-full rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-slate-400">IMG</div>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Navigation Menu */}
      <div className="bg-[#0d0d0d]/40 border-b border-white/5 backdrop-blur-xl">
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium transition-colors border-b-2 ${
                activeTab === item.id ? 'text-emerald-400 border-emerald-500' : 'text-slate-400 border-transparent hover:text-white hover:bg-white/5'
              }`}
            >
            ...
            <>
              ...
              <section>
                <h2 className="text-3xl font-black tracking-tighter text-white mb-6 flex items-center gap-2">
                  <span className="text-emerald-500">✦</span> Mis Credenciales
                </h2>

                {credentials.length === 0 ? (
                  <div className="text-center py-12 bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 rounded-2xl">
                    <p className="text-slate-400">No tienes credenciales registradas aún.</p>
                    <button
                      className="mt-4 px-6 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-lg hover:bg-emerald-500/30 transition-colors font-medium"
                      disabled={issuing}
                      onClick={async () => {
                        try {
                          setIssuing(true);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {credentials.map((cred) => (
                      <motion.div
                        ...
                        className="bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 rounded-2xl p-6 relative group hover:border-emerald-500/30 transition-all duration-300"
                      >
                        ...
                        <div className="mb-4">
                          <div className="text-xs text-emerald-400 font-mono mb-1">{cred.issuer}</div>
                          <h3 className="text-xl font-bold text-white line-clamp-2">{cred.title}</h3>
                          <div className="text-xs text-slate-500 mt-2">Emitido: {new Date(cred.issueDate).toLocaleDateString()}</div>
                        </div>

                        <div className="space-y-2 mt-4">
                          <div className="flex justify-between text-xs bg-black/40 p-2 rounded border border-white/5">
                            <span className="text-slate-400">Token ID</span>
                            <span className="font-mono text-emerald-300">{cred.metadata?.tokenId || 'Pending'}</span>
                          </div>
                          <div className="flex justify-between text-xs bg-black/40 p-2 rounded border border-white/5">
                            <span className="text-slate-400">Serial</span>
                            <span className="font-mono text-slate-300">#{cred.metadata?.serialNumber || '00'}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-6 flex gap-2">
                          <a
                            ...
                            className="flex-1 px-3 py-2 text-center text-xs border border-white/10 hover:border-emerald-500/50 rounded-lg text-slate-300 hover:text-white transition-colors"
                          >
                            HashScan
                          </a>
                          <button className="flex-1 px-3 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-lg text-xs font-bold hover:bg-emerald-500/30 transition-colors">
                            Verificar
                          </button>
                        </div>
                      </motion.div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 rounded-2xl p-6">
                  <h2 className="text-xl font-bold mb-4 text-white">Verificador Holográfico</h2>
                  ...
                  <div className="bg-black/40 rounded-xl p-4 min-h-[200px] flex items-center justify-center border border-white/10 border-dashed">
                    <CredentialVerifier />
                  </div>
                </div>

                <div className="bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 rounded-2xl p-6 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl -mr-10 -mt-10"></div>
                  <h2 className="text-xl font-bold mb-4 text-white">Portfolio Académico</h2>
                  <div className="space-y-3">
                    <button className="w-full text-left p-3 rounded-lg hover:bg-white/5 transition-colors border border-transparent hover:border-white/10 flex items-center justify-between group">
          ...
          {activeTab === 'marketplace' && (
            ...
            >
                <div className="text-center mb-8">
                    <h2 className="text-4xl font-black tracking-tighter text-white mb-2">Beneficios Exclusivos</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 rounded-2xl p-8 relative overflow-hidden">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2">
                                            <div className="relative flex-1">
                                                <Linkedin className="absolute left-3 top-3.5 text-slate-500" size={18} strokeWidth={1} />
                                                <input 
                                                    type="text" 
                                                    value={linkedInUrl}
                                                    ...
                                                    className={`w-full bg-black/40 border ${isLinkedInVerified ? 'border-green-500/30 text-green-400' : 'border-white/10 text-white'} rounded-lg py-3 pl-10 pr-4 focus:ring-2 focus:ring-emerald-500 outline-none transition-all`}
                                                />
                                                {isLinkedInVerified && (
                                                    <CheckCircle className="absolute right-3 top-3.5 text-green-500" size={18} strokeWidth={1} />
                                                )}
                                            </div>
                                            <button 
                                        ...
                                        {isLinkedInVerified ? (
                                            <>
                                                ...
                                                <span className="text-sm text-green-400 font-bold flex items-center gap-2">
                                                    Estado: Visible en Tech Recruiters Inc. <CheckCircle size={14} strokeWidth={1} />
                                                </span>
                                            </>
                                        ) : (
                                            <>
                    ...
                    <div className="lg:col-span-1 bg-[#0d0d0d]/40 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-6 relative overflow-hidden flex flex-col">
                      ...
                      <div className="w-8 h-8 rounded-lg bg-purple-500/20 flex items-center justify-center text-purple-400 shrink-0 group-hover:bg-purple-500 group-hover:text-white transition-colors border border-purple-500/20">
                        <FileText size={16} strokeWidth={1} />
                                    ...
                                    <ExternalLink size={14} className="text-slate-600 ml-auto group-hover:text-purple-400" strokeWidth={1} />
                                </Link>
                            </div>

                            <div className="flex gap-3">
                                <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-400 shrink-0 border border-blue-500/20">
                            ...
                            <div className="flex gap-3">
                                <div className="w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-500/20">
                        ...
                        <div className="mt-6 pt-6 border-t border-white/5">
                            <div className="bg-purple-900/20 rounded-lg p-3 border border-purple-500/20 mb-4">
                                <p className="text-xs text-purple-300 flex gap-2">
                                    <Award size={14} className="shrink-0 mt-0.5" strokeWidth={1} />
                                    <span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 opacity-60">
                    {['Mentoria IA', 'Networking Global', 'Becas Crypto'].map((benefit, i) => (
                        <div key={i} className="bg-[#0d0d0d]/40 backdrop-blur-xl border border-white/5 border-dashed rounded-2xl p-4">
                            <div className="h-8 w-8 bg-white/5 rounded mb-2"></div>
